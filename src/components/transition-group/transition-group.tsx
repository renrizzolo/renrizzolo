...
export class TransitionGroup implements ComponentInterface {
  ...
  mountWatch(newValue: boolean, oldValue: boolean) {
    ...
    if (newValue !== oldValue) {
      ...
      if (newValue === true) {
        console.log("mountWatcch > from > enter");

        this.setStyle(this.from, this.enter);
      }
      if (newValue === false) {
        console.log("!mountWatcch > leave");
  ...
  itemsWatch(newValue: any[], oldValue: any[]) {
    if (newValue) {
      ...
      if (newKeys && newKeys.length && !shallowEqual(newKeys, oldKeys)) {
        ...
        if (this.isMounted) {
          console.log("is mounted > leave");

          this.style = { ...this.leave };
        } else {
          console.log("!is mounted > from");
        ...
        const timeout =
          ...
          this.config.delay +
          this.config.delay * oldKeys.length;
        ...
        {this.initialItems.map((item, i) => {
          ...
          return (
            <div
              ...
              style={{
                ...style,
                transitionDelay: shouldNotDelay
                  ? "0ms"
                  : `${this.config.delay * i + 1}ms`,
